...
void setup(){
  size(800, 800); // Initialize the size of the window (need to change them manually here, variables not accepted by method)
...
void draw(){ // Method called every frame
  background(255);
  // Type instructions
  fill(0);
  textSize(32);
  ...
  if(drawCircle){ // We'll draw a circle by dragging the mouse
    ...
    if(decidingCircle){ // While we're deciding, the circle radius gets resized
      constructDots();
      circleWidth = (int)(dist(mouseX, mouseY, circleX, circleY)*2.0);
...
void constructDots(){ // Resets the dot colors to grey
...
void highlightDots(){
  ...
  int endRow = (int)Math.ceil(circleWidth/1.0/unitDisplacement) + startRow;
  int endCol = (int)Math.ceil(circleWidth/1.0/unitDisplacement) + startCol;
  if(endRow >=20)
    endRow = 19;
  if(endCol >=20)
    endCol = 19;
...
int convertToDot(int d){ // Converts x or y coordinates into the closest row or column number respectively
...
int convertToCor(int i){ // Converts row or column into x or y coordinates respectively
  return startCoordinates + (i * unitDisplacement);
}

void setMarginCircles(){ // Set the margin
...
float distancesFromCircle(ArrayList<PVector> points, PVector circle, float cWidth){
  float rad = cWidth/2;
  float total = 0;
...
float distanceFromCircle(float circX, float circY, float px, float py, float rad){ // Returns the distance between a point and the circle's edge
